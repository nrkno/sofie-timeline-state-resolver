...
import { VizMSECommandType, VizMSEState } from '../types'
import { ExpectedPlayoutItemContentBase } from '../../../expectedPlayoutItems'

jest.mock('@tv2media/v-connection')
jest.mock('net')

// Some 'hacks' to get the mocks correctly registered
import * as vConnectionLib from '@tv2media/v-connection'
jest.spyOn(vConnectionLib, 'createMSE').mockImplementation(vConnection.createMSE)
...
type VIZMSEPlayoutItemContentInternalFull = VIZMSEPlayoutItemContentInternal & ExpectedPlayoutItemContentBase
...
const defaultPlaylistId = 'my-super-playlist-id'
...
async function setupDevice() {
	...
	const device = new VizMSEDevice(getDeviceContext())
	await device.init({
		host: '127.0.0.1',
		preloadAllElements: true,
	...
	return { device, myChannelMapping }
}
...
describe('vizMSE', () => {
	...
	// 	const { device, myChannelMapping } = await setupDevice()
	// 	await mockTime.advanceTimeToTicks(10100)

	// 	// // mock the sendCommand function:
	// 	// const mockSendCommand = jest.fn<ReturnType<typeof device.sendCommand>, Parameters<typeof device.sendCommand>>(
	// 	// 	async () => {
	// 	// 		// No-op
	// 	// 	}
	// 	// )
	// 	// device.sendCommand = mockSendCommand
	...
	// 	const mse = vConnection.getLastMockMSE()
	...
	// test.only('re-initializes show for incoming elements during TimelineObjVIZMSEInitializeShows', async () => {
	// 	const { device, myChannelMapping } = await setupDevice()
	// 	device.ignoreWaitsInTests()
	...
	// 	device.handleExpectedPlayoutItems(
	// 		literal<VIZMSEPlayoutItemContentInternalFull[]>([
	// 			{
	// 				rundownId: '',
	// 				playlistId: defaultPlaylistId,
	// 				templateName: 'bund',
	...
	// 		command: {
	// 			time: mockTime.now + 5000, // 15100
	// 			type: VizMSECommandType.INITIALIZE_SHOWS,
	// 			showIds: [MOCK_SHOWS[0].name, MOCK_SHOWS[1].name],
	// 			timelineObjId: 'obj0',
	// 		},
	// 		context: 'MOCK',
	// 		timelineObjId: 'obj0',
