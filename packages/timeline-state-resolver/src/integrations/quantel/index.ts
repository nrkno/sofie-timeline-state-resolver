 ...
 */
export class QuantelDevice extends AbstractStateDevice<QuantelState, DeviceOptionsQuantelInternal> {
	...
	constructor(deviceId: string, deviceOptions: DeviceOptionsQuantelInternal, getCurrentTime: () => Promise<number>) {
		...
		this._quantel.on('error', (e) => this.emitLog('error', 'Quantel.QuantelGateway', e))
		...
		this._quantelManager.on('info', (x) =>
			this.emitLog('info', `Quantel: ${typeof x === 'string' ? x : JSON.stringify(x)}`)
		)
		this._quantelManager.on('warning', (x) =>
			this.emitLog('warning', `Quantel: ${typeof x === 'string' ? x : JSON.stringify(x)}`)
		)
		this._quantelManager.on('error', (e) => this.emitLog('error', 'Quantel: ', e))
		this._quantelManager.on('debug', (...args) => this.emitLog('debug', ...args))
	...
	async init(): Promise<boolean> {
		this._initOptions = this.getOptions()
	...
	handleState(newState: TimelineState, newMappings: Mappings) {
		...
		if (!this._quantel.initialized) {
			this.emitLog('warning', 'Quantel not initialized yet')
			return
		}
